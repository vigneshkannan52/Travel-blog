 ...
 * Register widget
 */
function explore_blog_register_widget() {
	register_widget( 'Explore_Blog_Social_Icons_Widget' );
}
add_action( 'widgets_init', 'explore_blog_register_widget' );
 ...
 * @since 1.0.0
 */
class Explore_Blog_Social_Icons_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			// widget ID.
			'explore_blog_social_icons',
			// widget name.
			__( 'Ascendoor: Social Icons', 'explore-blog' ),
			// widget description.
			array( 'description' => __( 'Adds a social icon menu.', 'explore-blog' ) )
		);
	}
	...
	public function form( $instance ) {
		...
		if ( ! $menus ) {
			...
			echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'explore-blog' ), esc_url( admin_url( 'nav-menus.php' ) ) ) . '</p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return;
		}
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'explore-blog' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>"><?php esc_html_e( 'Select Menu:', 'explore-blog' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'nav_menu' ) ); ?>">
				<option value="0"><?php esc_html_e( '&mdash; Select &mdash;', 'explore-blog' ); ?></option>
				<?php
